...
def run_rl_ga(config: Dict):
    ...
    def scoring_function(
        ...
        unroll_fn = functools.partial(
            generate_unroll,
            episode_length=1000,
            play_step_fn=ensemble_gp_play_step_fn,
        )
    ...
    def vmap_mutate(population_of_genotypes, mutation_key):
        this_n_pop = jax.tree_util.tree_leaves(population_of_genotypes)[0].shape[0]
        multi_mutate_keys = jax.random.split(mutation_key, this_n_pop)
...
if __name__ == '__main__':
    ...
    for seed in range(10):
        conf["seed"] = seed
        conf["n_gens"] = n_gens
        conf["run_name"] = "E-CGP_" + conf["problem"].replace("/", "_") + "_" + str(conf["seed"])
        print(conf["run_name"])
